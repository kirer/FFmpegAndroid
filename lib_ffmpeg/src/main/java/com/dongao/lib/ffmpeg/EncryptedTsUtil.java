...
final class EncryptedTsUtil {

    private static final int AES_BLOCK_SIZE = 16;
    private static final int BUFFER_SIZE = 16 * 1024;

    private EncryptedTsUtil() {
    }

    static void decryptFile(File input, File output, String keyInput, String ivInput) throws Exception {
        transform(input, output, Cipher.DECRYPT_MODE, keyInput, ivInput);
    }

    static void encryptFile(File input, File output, String keyInput, String ivInput) throws Exception {
    ...
    private static byte[] normalizeAesBytes(String value, String label) {
        if (value == null) {
        ...
        throw new IllegalArgumentException(String.format(Locale.US,
                "%s must be 16 raw bytes or 32 hex chars, but was %d chars",
